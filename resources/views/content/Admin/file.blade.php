    ...
    <div class="container px-2 mx-10 justify-center items-center">
        <div class="relative">
                                ...
                                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6 mb-3">
                                    ...
                                    <div class="hidden sm:col-span-2">
                                        ...
                                        <input type="text" name="role" id="disabled-input-2"
                                            ...
                                            value="{{ Auth::user()->role ?? Auth::guard('staf')->user()->role }}" disabled
                                            readonly>
                                    </div>
                                </div>
            ...
            <div class="lg:hidden overflow-x-scroll">
                ...
                <table class=" mt-5 text-sm text-left rtl:text-right text-gray-500  table-auto stripe" id="ls">
                            ...
                            <tr class="bg-white border-b ">
                                ...
                                <td class="px-2 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <form action="{{ route('deleteFile', $view->id) }}" method="POST">
                                        <a title="Download" href="uploads/documents/{{ $view->nama_file }}"
                                            class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-xs px-2 no-underline py-2 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                                            download="uploads/documents/{{ $view->nama_file }}"><i
                                                class="mdi mdi-download" style="font-size: 18px"></i></a>
                                        @csrf
                                        @method('DELETE')
            ...
            <div class="hidden lg:block">
                ...
                <table
                    class=" mt-5 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-auto stripe"
                    id="dt">
                        ...
                        @foreach ($folder as $view)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                    <form action="{{ route('deleteFile', $view->id) }}" method="POST">
                                        <a title="Download"
                                            href="uploads/documents/{{ $view->nama_file }}"
                                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline btn btn-success"
                                            download="uploads/documents/{{ $view->nama_file }}">Download</a>
                                        @csrf
                                        @method('DELETE')
                                        <button title="Hapus" type="submit" onclick="return confirm('Yakin ingin menghapus?')"
